    ...
    while (i + 4 <= buf.length) {
        ...
        if (sig === 0x04034b50) {
            ...
            if (method === 0) {
                ...
                result.set(name, await inflate(compressed));
            }
...
// ── list_projects ───────────────────────────────────────────────────────────
server.tool("list_projects", "List all registered projects in this ContextForge instance, ordered by most recently created.", {}, async () => {
    const db = openDb(true);
    try {
        const rows = db.prepare("SELECT id, name, project_type, description, created_at FROM projects ORDER BY created_at DESC").all();
        return { content: [{ type: "text", text: JSON.stringify({ projects: rows, count: rows.length }, null, 2) }] };
    }
    finally {
        db.close();
    }
...
// ── init_project ────────────────────────────────────────────────────────────
server.tool("init_project", "Create or update a project. If project_id already exists, metadata is updated — no data is lost.", {
    ...
    try {
      ...
      `).run(project_id, name, project_type, description, JSON.stringify(goals), JSON.stringify(tech_stack), JSON.stringify(constraints), nowIso(), nowIso());
        return { content: [{ type: "text", text: JSON.stringify({ status: "ok", project_id, name }) }] };
    }
    finally {
        db.close();
    }
...
// ── rename_project ──────────────────────────────────────────────────────────
server.tool("rename_project", "Rename a project display name. The project_id slug does not change.", {
    ...
    try {
        ...
        if (!exists)
            return { content: [{ type: "text", text: JSON.stringify({ error: `Project '${project_id}' not found` }) }] };
        ...
        return { content: [{ type: "text", text: JSON.stringify({ status: "renamed", project_id, new_name }) }] };
    }
    finally {
        db.close();
    }
...
// ── merge_projects ──────────────────────────────────────────────────────────
server.tool("merge_projects", "Merge source_project_id INTO target_project_id. All nodes, tasks, and archives are re-assigned. Source is deleted. Irreversible — snapshot first.", {
    ...
    if (source_project_id === target_project_id) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "source and target must be different projects" }) }] };
    }
    const db = openDb(false);
    try {
        ...
        if (!src)
            return { content: [{ type: "text", text: JSON.stringify({ error: `Source project '${source_project_id}' not found` }) }] };
        if (!tgt)
            return { content: [{ type: "text", text: JSON.stringify({ error: `Target project '${target_project_id}' not found` }) }] };
        ...
        const result = merge();
        return { content: [{ type: "text", text: JSON.stringify({ status: "merged", source: source_project_id, target: target_project_id, ...result }) }] };
    }
    finally {
        db.close();
    }
...
// ── delete_project ──────────────────────────────────────────────────────────
server.tool("delete_project", "Permanently delete a project. Active nodes are archived to historical_nodes first (unless archive_nodes=false). Irreversible.", {
    project_id: z.string().min(2),
    ...
    try {
        ...
        const del = db.transaction(() => {
            let archived = 0;
            if (archive_nodes) {
                const nodes = db.prepare("SELECT * FROM decision_nodes WHERE project_id = ? AND tombstone = 0").all(project_id);
              ...
              VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
            `).run(crypto.randomUUID(), n.id, project_id, n.summary, n.rationale, n.area, n.confidence, n.status, n.created_by_agent, "mcp-delete", "project deleted via MCP", n.created_at, n.type_metadata ?? "{}");
                    archived++;
                }
            }
        ...
        const result = del();
        return { content: [{ type: "text", text: JSON.stringify({ status: "deleted", project_id, ...result }) }] };
    }
    finally {
        db.close();
    }
...
// ── project_stats ───────────────────────────────────────────────────────────
server.tool("project_stats", "Return statistics for a project: node counts by area and status, task completion.", { project_id: z.string().min(2) }, async ({ project_id }) => {
    const db = openDb(true);
    try {
        const proj = db.prepare("SELECT * FROM projects WHERE id = ?").get(project_id);
        if (!proj)
            return { content: [{ type: "text", text: JSON.stringify({ error: `Project '${project_id}' not found` }) }] };
        const activeNodes = db.prepare("SELECT COUNT(*) as cnt FROM decision_nodes WHERE project_id = ? AND tombstone = 0 AND status = 'active'").get(project_id).cnt;
        const totalNodes = db.prepare("SELECT COUNT(*) as cnt FROM decision_nodes WHERE project_id = ? AND tombstone = 0").get(project_id).cnt;
        const areaRows = db.prepare("SELECT area, COUNT(*) as cnt FROM decision_nodes WHERE project_id = ? AND tombstone = 0 AND status = 'active' GROUP BY area ORDER BY cnt DESC").all(project_id);
        const taskRows = db.prepare("SELECT status, COUNT(*) as cnt FROM tasks WHERE project_id = ? GROUP BY status").all(project_id);
        const taskCounts = Object.fromEntries(taskRows.map(r => [r.status, r.cnt]));
...
server.tool("capture_decision", "Append a decision node to the knowledge graph. Records WHY a decision was made, alternatives considered, and causal dependencies.", {
    project_id: z.string().min(2),
    ...
    try {
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      `).run(node_id, project_id, summary, rationale, area, JSON.stringify(alternatives), JSON.stringify([]), confidence, 0.5, JSON.stringify({}), "mcp-ts-client", 0, "mcp-ts-client", "active", JSON.stringify({ file_refs, packages: [] }), now, now);
...
// ── load_context ────────────────────────────────────────────────────────────
server.tool("load_context", "Load hierarchical context for a project. L0=abstract metadata; L1=+decision summaries; L2=+full rationale+alternatives.", {
    project_id: z.string().min(2),
    ...
    try {
        ...
        const clauses = ["project_id = ?", "tombstone = 0", "status = 'active'"];
        ...
        params.push(top_k);
        const rows = db.prepare(`SELECT * FROM decision_nodes WHERE ${clauses.join(" AND ")} ORDER BY importance DESC, created_at DESC LIMIT ?`).all(...params);
        if (detail_level === "L1") {
            return { content: [{ type: "text", text: JSON.stringify({
                            ...l0, level: "L1",
...
// ── get_knowledge_node ──────────────────────────────────────────────────────
server.tool("get_knowledge_node", "Retrieve a decision node by UUID or keyword search. Returns WHY the decision was made at the requested detail level.", {
    ...
    try {
        let rows;
        if (node_id) {
            const row = db.prepare("SELECT * FROM decision_nodes WHERE id = ? AND tombstone = 0").get(node_id);
            if (!row)
            ...
            params.push(top_k);
            rows = db.prepare(`SELECT * FROM decision_nodes WHERE ${clauses.join(" AND ")} ORDER BY importance DESC LIMIT ?`).all(...params);
        }
        else {
            return { content: [{ type: "text", text: JSON.stringify({ error: "Provide node_id or query" }) }] };
        }
        const format = (r) => {
            if (detail_level === "L0")
                return `[${r.area}] ${r.summary}`;
            if (detail_level === "L1")
                return { id: r.id, area: r.area, summary: r.summary, rationale: r.rationale, confidence: r.confidence, status: r.status };
            return {
                id: r.id, area: r.area, summary: r.summary, rationale: r.rationale,
                alternatives: parseJson(r.alternatives, []),
                dependencies: parseJson(r.dependencies, []),
...
// ── list_decisions ──────────────────────────────────────────────────────────
server.tool("list_decisions", "List decision nodes for a project with optional area and status filters.", {
    project_id: z.string().min(2),
    ...
    try {
        ...
        params.push(limit);
        const rows = db.prepare(`SELECT id, area, summary, rationale, confidence, status, created_at FROM decision_nodes WHERE ${clauses.join(" AND ")} ORDER BY importance DESC LIMIT ?`).all(...params);
        return { content: [{ type: "text", text: JSON.stringify({ project_id, count: rows.length, decisions: rows }, null, 2) }] };
    }
    finally {
        db.close();
    }
...
// ── update_decision ─────────────────────────────────────────────────────────
server.tool("update_decision", "Edit fields on an existing decision node (summary, rationale, area, confidence, importance).", {
    ...
    const db = openDb(false);
    try {
        const allowed = ["summary", "rationale", "area", "confidence", "importance"];
        const updates = allowed.filter(k => fields[k] !== undefined);
        if (updates.length === 0) {
            return { content: [{ type: "text", text: JSON.stringify({ error: "No updatable fields provided (allowed: summary, rationale, area, confidence, importance)" }) }] };
        }
        ...
        if (result.changes === 0)
            return { content: [{ type: "text", text: JSON.stringify({ error: `Decision node '${node_id}' not found` }) }] };
        return { content: [{ type: "text", text: JSON.stringify({ status: "updated", node_id, updated_fields: updates }) }] };
    }
    finally {
        db.close();
    }
...
// ── deprecate_decision ──────────────────────────────────────────────────────
server.tool("deprecate_decision", "Mark a decision node as deprecated with a reason and optional replacement node ID.", {
    ...
    try {
        let result;
        if (replacement_id) {
            result = db.prepare("UPDATE decision_nodes SET status = 'deprecated', deprecated_reason = ?, replacement_id = ?, updated_at = ? WHERE id = ? AND tombstone = 0").run(reason, replacement_id, nowIso(), node_id);
        }
        else {
            result = db.prepare("UPDATE decision_nodes SET status = 'deprecated', deprecated_reason = ?, updated_at = ? WHERE id = ? AND tombstone = 0").run(reason, nowIso(), node_id);
        }
        if (result.changes === 0)
            return { content: [{ type: "text", text: JSON.stringify({ error: `Decision node '${node_id}' not found` }) }] };
        return { content: [{ type: "text", text: JSON.stringify({ status: "deprecated", node_id, reason, replacement_id: replacement_id ?? null }) }] };
    }
    finally {
        db.close();
    }
...
// ── link_decisions ──────────────────────────────────────────────────────────
server.tool("link_decisions", "Create a typed edge between two decision nodes (depends_on | replaces | contradicts | refines | implements).", {
    ...
    try {
        ...
        db.prepare("INSERT INTO decision_edges (id, source_id, target_id, edge_type) VALUES (?, ?, ?, ?)").run(edge_id, source_id, target_id, edge_type);
        return { content: [{ type: "text", text: JSON.stringify({ status: "linked", edge_id, source: source_id, target: target_id, type: edge_type }) }] };
    }
    finally {
        db.close();
    }
...
// ── list_tasks ──────────────────────────────────────────────────────────────
server.tool("list_tasks", "List tasks for a project with optional status filter.", {
    project_id: z.string().min(2),
    ...
    try {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ project_id, count: rows.length, tasks: rows }, null, 2) }] };
    }
    finally {
        db.close();
    }
...
// ── create_task ─────────────────────────────────────────────────────────────
server.tool("create_task", "Create a new task in a project. Auto-creates a stub project if project_id does not exist.", {
    project_id: z.string().min(2),
    ...
    try {
        ...
        const now = nowIso();
        db.prepare("INSERT INTO tasks (id, project_id, title, description, status, priority, sprint, assigned_to, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").run(task_id, project_id, title, description, "pending", priority, sprint, assigned_to, now, now);
        return { content: [{ type: "text", text: JSON.stringify({ status: "created", task_id, title }) }] };
    }
    finally {
        db.close();
    }
...
// ── update_task ─────────────────────────────────────────────────────────────
server.tool("update_task", "Update the status of an existing task.", {
    task_id: z.string(),
    ...
    try {
        ...
        if (result.changes === 0)
            return { content: [{ type: "text", text: JSON.stringify({ error: `Task '${task_id}' not found` }) }] };
...
// ── rollback ────────────────────────────────────────────────────────────────
server.tool("rollback", "Time-travel undo: mark all events after a given event_id or timestamp as rolled_back. Returns the count of pruned events.", {
    ...
    if (!event_id && !timestamp) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "Provide either event_id or timestamp" }) }] };
    }
    const db = openDb(false);
    try {
        let cutoff;
        if (event_id) {
            const row = db.prepare("SELECT created_at FROM events WHERE event_id = ?").get(event_id);
            if (!row)
                return { content: [{ type: "text", text: JSON.stringify({ error: `event_id '${event_id}' not found` }) }] };
        ...
        let result;
        if (project_id) {
            result = db.prepare("UPDATE events SET status = 'rolled_back' WHERE created_at > ? AND status = 'active' AND (project_id = ? OR project_id IS NULL)").run(cutoff, project_id);
        }
        else {
            result = db.prepare("UPDATE events SET status = 'rolled_back' WHERE created_at > ? AND status = 'active'").run(cutoff);
        }
        // Insert a ROLLBACK event to record the operation
        const rollback_id = crypto.randomUUID();
        db.prepare("INSERT INTO events (event_id, event_type, content, status, created_at, project_id) VALUES (?,?,?,?,?,?)").run(rollback_id, "ROLLBACK", JSON.stringify({ pruned: result.changes, cutoff, by: "mcp-ts-client" }), "active", nowIso(), project_id ?? null);
        return { content: [{ type: "text", text: JSON.stringify({ pruned_events: result.changes, status: "rolled_back", cutoff }) }] };
    }
    finally {
        db.close();
    }
...
// ── snapshot ────────────────────────────────────────────────────────────────
server.tool("snapshot", "Create an AES-256-GCM encrypted .forge snapshot of the event ledger. Compatible with Python FluidSync format.", {
    ...
    try {
        ...
        const db = openDb(true);
        const events = db.prepare("SELECT * FROM events WHERE status = 'active' ORDER BY created_at ASC").all();
        db.close();
        const eventsJson = Buffer.from(JSON.stringify(events, null, 2), "utf8");
        // Read charter if present
        let charterBytes = Buffer.alloc(0);
        try {
            charterBytes = fs.readFileSync(CHARTER_PATH);
        }
        catch { /* ok */ }
        ...
        return { content: [{ type: "text", text: JSON.stringify({ status: "ok", snapshot_path: snapPath, event_count: events.length, size_bytes: encrypted.length }) }] };
    }
    catch (e) {
...
// ── list_snapshots ──────────────────────────────────────────────────────────
server.tool("list_snapshots", "List all .forge snapshot files in the .forge/ directory with metadata.", {}, async () => {
    try {
        fs.mkdirSync(FORGE_DIR, { recursive: true });
        ...
        return { content: [{ type: "text", text: JSON.stringify({ snapshots: files, count: files.length }, null, 2) }] };
    }
    catch (e) {
        return { content: [{ type: "text", text: JSON.stringify({ error: String(e) }) }] };
    }
...
// ── replay_sync ─────────────────────────────────────────────────────────────
server.tool("replay_sync", "Restore events from a .forge snapshot. Events already in the ledger are skipped (idempotent). Returns count of replayed events.", {
    forge_path: z.string().describe("Path to the .forge snapshot file"),
}, async ({ forge_path }) => {
    try {
        if (!fs.existsSync(forge_path)) {
            return { content: [{ type: "text", text: JSON.stringify({ error: `.forge file not found: ${forge_path}` }) }] };
        }
        ...
        if (!eventsRaw)
            return { content: [{ type: "text", text: JSON.stringify({ error: "Invalid .forge file: missing events.json" }) }] };
        const events = JSON.parse(eventsRaw.toString("utf8"));
        ...
        const db = openDb(false);
        try {
            const existing = new Set(db.prepare("SELECT event_id FROM events").all().map(r => r.event_id));
            let replayed = 0;
            const insert = db.prepare("INSERT OR IGNORE INTO events (event_id, parent_id, event_type, content, metadata, status, created_at, prev_hash, project_id) VALUES (?,?,?,?,?,?,?,?,?)");
            ...
            const replay = db.transaction(() => {
                for (const evt of ordered) {
                    ...
                    insert.run(eid, evt["parent_id"] ?? null, evt["event_type"] ?? "AGENT_THOUGHT", content, typeof evt["metadata"] === "string" ? evt["metadata"] : JSON.stringify(evt["metadata"] ?? {}), "active", evt["created_at"] ?? nowIso(), evt["prev_hash"] ?? null, evt["project_id"] ?? null);
                    replayed++;
                }
                return replayed;
            });
            const count = replay();
            return { content: [{ type: "text", text: JSON.stringify({ status: "synced", replayed_events: count, total_in_snapshot: events.length }) }] };
        }
        finally {
            db.close();
        }
...
server.tool("list_events", "Inspect the event ledger — append-only record of all agent activity.", {
    ...
    try {
        ...
        const rows = db.prepare(`SELECT event_id, event_type, status, created_at, project_id, content FROM events ${where} ORDER BY created_at DESC LIMIT ?`).all(...params);
...
// ── search_context ──────────────────────────────────────────────────────────
server.tool("search_context", "Keyword search over local project files. Searches file contents using fuzzy word matching. For full semantic search, use the Python server.", {
    ...
    if (terms.length === 0) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "Query too short — use at least one word with 3+ characters" }) }] };
    }
    const results = [];
    function crawl(dir) {
        let entries;
    ...
    return { content: [{ type: "text", text: JSON.stringify({ query, count: top.length, results: top }, null, 2) }] };
});
// ── Start ───────────────────────────────────────────────────────────────────
async function main() {
